...
class ConfigMigrationTest {
    ...
    @AfterTest
    fun tearDown() {
        java.nio.file.Path.of(dir.toString()).deleteRecursively()
    }

    @Test
    fun migration_copiesRowsToFiles_andRunsOnce() = runBlocking {
        ...
        assertEquals("char hl", HighlightRepositoryImpl(store).observeByCharacter(charId).first().single().pattern)
    }
}

private class FakeCharacterDao(private val all: List<CharacterEntity>) : CharacterDao {
...
private class FakeHighlightDao(private val byCharacter: Map<String, List<PopulatedHighlight>>) : HighlightDao {
    override suspend fun getHighlightsByCharacter(characterId: String): List<PopulatedHighlight> =
        byCharacter[characterId] ?: emptyList()
    ...
    override suspend fun deleteByPattern(pattern: String, characterId: String) = error("unused")
...
private class FakeNameDao(private val byCharacter: Map<String, List<NameEntity>>) : NameDao {
    ...
    override suspend fun deleteByText(text: String, characterId: String) = error("unused")
...
private class FakeVariableDao(private val byCharacter: Map<String, List<VariableEntity>>) : VariableDao {
    ...
    override suspend fun delete(characterId: String, name: String) = error("unused")
