...
class CharacterConfigStoreTest {
    ...
    @Test
    fun highlight_saved_isPersistedAndReloadable() = runBlocking {
        val store = newStore()
        store.load()
        ...
        val file = Path(Path(configDir, "characters"), "gs4_tholan.toml")
    ...
    @Test
    fun highlight_deleteById_findsOwningCharacter() = runBlocking {
        val store = newStore()
        store.load()
        val repo = HighlightRepositoryImpl(store)
    ...
    @Test
    fun highlight_observeForCharacter_mergesGlobalAndCharacter() = runBlocking {
        val store = newStore()
        store.load()
        val repo = HighlightRepositoryImpl(store)
        ...
        val merged = repo.observeForCharacter("gs4:tholan").first().map { it.pattern }.toSet()
        assertEquals(setOf("global one", "character one"), merged)
    }

    @Test
    fun highlight_upsert_replacesSamePattern() = runBlocking {
        val store = newStore()
        store.load()
        val repo = HighlightRepositoryImpl(store)
    ...
    @Test
    fun name_roundTrips() = runBlocking {
        val store = newStore()
        store.load()
    ...
    @Test
    fun variables_putObserveDelete() = runBlocking {
        val store = newStore()
        store.load()
        ...
        assertEquals(
            "100",
            VariableRepository(reloaded).observeCharacterVariables("gs4:tholan").first().single().value,
        )
    }

    @Test
    fun handAuthoredEntryWithoutId_getsStableIdOnLoad() = runBlocking {
        // Simulate a user hand-writing a file with no id field.
        val charsDir = Path(configDir, "characters")
        fs.createDirectories(charsDir)
        val file = Path(charsDir, "gs4_tholan.toml")
    ...
    @Test
    fun emptyStore_observesEmpty() = runBlocking {
        val store = newStore()
        store.load()
