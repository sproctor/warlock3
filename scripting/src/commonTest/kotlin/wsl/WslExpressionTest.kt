...
class WslExpressionTest {
    private fun parseExpression(content: String): WslExpression {
        ...
        try {
            ...
            return command.contents.filterIsInstance<WslCommandContent.Expression>().single().expression
        } finally {
    ...
    @Test
    fun numericEquality() = runTest {
        assertTrue(backgroundScope.eval("1 = 1").toBoolean())
        assertFalse(backgroundScope.eval("1 = 2").toBoolean())
    }

    @Test
    fun inequality() = runTest {
        assertTrue(backgroundScope.eval("1 != 2").toBoolean())
        assertFalse(backgroundScope.eval("1 != 1").toBoolean())
    }

    @Test
    fun stringEqualityIsCaseInsensitive() = runTest {
    ...
    @Test
    fun chainedComparisonsWithAnd() = runTest {
    ...
    @Test
    fun existsIsTrueForSetVariable() = runTest {
        val result =
    ...
    @Test
    fun existsIsFalseForUnsetVariable() = runTest {
    ...
    @Test
    fun variableReferenceInExpression() = runTest {
        val result =
            backgroundScope.eval("x + 1") {
                setScriptVariableRaw("x", WslNumber(com.ionspin.kotlin.bignum.decimal.BigDecimal.fromInt(5)))
            }
        assertEquals("6", result.toString())
    }

    @Test
    fun mapIndexingInExpression() = runTest {
        val result =
    ...
    @Test
    fun unknownVariableIsNull() = runTest {
