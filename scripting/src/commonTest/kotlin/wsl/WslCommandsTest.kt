...
class WslCommandsTest {
    ...
    @Test
    fun varSetsScriptVariable() = runTest {
    ...
    @Test
    fun varWithBlankNameThrows() = runTest {
        val ctx = buildTestContext(backgroundScope)
        assertFailsWith<WslRuntimeException> { ctx.command("var", "") }
    }

    @Test
    fun unsetvarRemovesScriptVariable() = runTest {
        val ctx = buildTestContext(backgroundScope)
        ctx.command("var", "name value")
    ...
    @Test
    fun localSetsLocalVariable() = runTest {
        val ctx = buildTestContext(backgroundScope)
    ...
    @Test
    fun unsetlocalRemovesLocalVariable() = runTest {
        val ctx = buildTestContext(backgroundScope)
        ctx.command("local", "name value")
    ...
    @Test
    fun counterAddWithoutOperandIncrementsByOne() = runTest {
        val ctx = buildTestContext(backgroundScope)
    ...
    @Test
    fun counterAddSubtractMultiplyDivide() = runTest {
        val ctx = buildTestContext(backgroundScope)
        ctx.command("counter", "set 10")
    ...
    @Test
    fun counterDivideByZeroThrows() = runTest {
        val ctx = buildTestContext(backgroundScope)
        ctx.command("counter", "set 10")
    ...
    @Test
    fun counterUnsupportedOperatorThrows() = runTest {
        val ctx = buildTestContext(backgroundScope)
        assertFailsWith<WslRuntimeException> { ctx.command("counter", "bogus 5") }
    }

    @Test
    fun counterNonNumericOperandThrows() = runTest {
        val ctx = buildTestContext(backgroundScope)
    ...
    @Test
    fun randomSetsRWithinRange() = runTest {
        val ctx = buildTestContext(backgroundScope)
        repeat(20) {
    ...
    @Test
    fun randomWithMinNotLessThanMaxThrows() = runTest {
        val ctx = buildTestContext(backgroundScope)
        assertFailsWith<WslRuntimeException> { ctx.command("random", "10 5") }
    }

    @Test
    fun randomWithInvalidArgsThrows() = runTest {
        val ctx = buildTestContext(backgroundScope)
    ...
    @Test
    fun shiftMovesNumberedVariablesDown() = runTest {
        val ctx = buildTestContext(backgroundScope)
    ...
    @Test
    fun mapAddCreatesMapAndSetsProperty() = runTest {
        val ctx = buildTestContext(backgroundScope)
    ...
    @Test
    fun mapAddOnNonMapThrows() = runTest {
        val ctx = buildTestContext(backgroundScope)
    ...
    @Test
    fun mapAddWithMissingArgumentsThrows() = runTest {
        val ctx = buildTestContext(backgroundScope)
        assertFailsWith<WslRuntimeException> { ctx.command("mapadd", "onlyname") }
    }

    @Test
    fun setArrayPopulatesIndexedProperties() = runTest {
        val ctx = buildTestContext(backgroundScope)
    ...
    @Test
    fun gotoWithBlankLabelThrows() = runTest {
        val ctx = buildTestContext(backgroundScope)
        assertFailsWith<WslRuntimeException> { ctx.command("goto", "") }
    }

    @Test
    fun gosubSetsArguments() = runTest {
        val lines = parseLines("echo a\nsub: echo b")
        val ctx = buildTestContext(backgroundScope, lines = lines)
    ...
    @Test
    fun ifNExecutesCommandWhenVariableExists() = runTest {
        val ctx = buildTestContext(backgroundScope)
    ...
    @Test
    fun ifNSkipsCommandWhenVariableMissing() = runTest {
        val ctx = buildTestContext(backgroundScope)
    ...
    @Test
    fun timerStartCreatesTimerAndClearRemovesIt() = runTest {
        val ctx = buildTestContext(backgroundScope)
    ...
    @Test
    fun echoPrintsToClient() = runTest {
    ...
    @Test
    fun sendForwardsCommand() = runTest {
        val client = FakeWarlockClient()
        val ctx = buildTestContext(backgroundScope, client = client)
    ...
    @Test
    fun putForwardsCommand() = runTest {
        val client = FakeWarlockClient()
        val ctx = buildTestContext(backgroundScope, client = client)
    ...
    @Test
    fun playPlaysSoundAndEchoes() = runTest {
        val client = FakeWarlockClient()
        ...
        assertTrue(client.printed.single().toString().contains("alert.wav"))
    }

    // --- logging ---

    @Test
    fun debugIsSuppressedAtDefaultLevelButInfoIsShown() = runTest {
        val client = FakeWarlockClient()
        val ctx = buildTestContext(backgroundScope, client = client)
    ...
    @Test
    fun debugLevelLowersThreshold() = runTest {
        val client = FakeWarlockClient()
        val ctx = buildTestContext(backgroundScope, client = client)
    ...
    @Test
    fun addToHighlightStringsSavesHighlight() = runTest {
    ...
    @Test
    fun addToHighlightNamesSavesName() = runTest {
    ...
    @Test
    fun deleteFromHighlightNamesDeletesByText() = runTest {
        val nameRepository = FakeNameRepository()
        val client = FakeWarlockClient(characterId = "testchar")
        val ctx = buildTestContext(backgroundScope, client = client, nameRepository = nameRepository)
    ...
    @Test
    fun waitForReWithInvalidRegexThrows() = runTest {
        val ctx = buildTestContext(backgroundScope)
    ...
    @Test
    fun addTextListenerWithoutNameThrows() = runTest {
        val ctx = buildTestContext(backgroundScope)
    ...
    @Test
    fun addTextListenerSetsVariableOnMatchingText() = runTest(UnconfinedTestDispatcher()) {
        val client = FakeWarlockClient()
        val ctx = buildTestContext(backgroundScope, client = client)
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun addTextListenerIgnoresNonMatchingText() = runTest(UnconfinedTestDispatcher()) {
        val client = FakeWarlockClient()
        val ctx = buildTestContext(backgroundScope, client = client)
        ctx.command("addtextlistener", "found hello")
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun addTextListenerReCapturesRegexMatch() = runTest(UnconfinedTestDispatcher()) {
        val client = FakeWarlockClient()
        val ctx = buildTestContext(backgroundScope, client = client)
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun removeTextListenerStopsUpdates() = runTest(UnconfinedTestDispatcher()) {
        val client = FakeWarlockClient()
        val ctx = buildTestContext(backgroundScope, client = client)
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun clearTextListenersStopsUpdates() = runTest(UnconfinedTestDispatcher()) {
        val client = FakeWarlockClient()
        val ctx = buildTestContext(backgroundScope, client = client)
        ctx.command("addtextlistener", "found")
