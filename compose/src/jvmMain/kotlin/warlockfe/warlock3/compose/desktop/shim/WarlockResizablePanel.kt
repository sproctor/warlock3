...
import warlockfe.warlock3.compose.components.getResizeCursor

@Composable
fun WarlockResizablePanel(
...
private fun PanelContent(
    ...
    if (handleBefore && showHandle) {
        WarlockResizablePanelHandle(isHorizontal, true, state)
    }
    Box(modifier) {
        content()
    }
    if (!handleBefore && showHandle) {
        WarlockResizablePanelHandle(isHorizontal, false, state)
    }
}

@Composable
fun WarlockResizablePanelHandle(
    isHorizontal: Boolean,
    isBefore: Boolean,
    state: ResizablePanelState,
    modifier: Modifier = Modifier,
    ...
    val handleColor = JewelTheme.globalColors.text.normal
