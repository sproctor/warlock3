...
/**
 * The shared mobile (Material3) editor for one text style at one edit scope, the twin of the desktop
 * `DesktopTextStyleEditor`. Each attribute shows its control, a "set here"/"inherited" tag, and a reset
 * enabled only when the value was set at [editScope]; a live sample renders the resolved cascade. Every
 * change folds into [editLayer] via [applyEdit] and is handed back through [onSave] for the caller to
 * persist to that one scope's layer.
 */
@Composable
fun TextStyleEditor(
    ...
    Column(modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(6.dp)) {
        ...
        if (showFont) {
            ...
            AttributeRow("Font", sourced.fontFamily.source, editScope, onReset = {
                ...
                OutlinedButton(onClick = { editFont = true }) {
...
private fun AttributeRow(
    ...
    Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        OutlinedButton(onClick = onReset, enabled = source == editScope) {
            Text("Reset")
        }
    }
}
